...
router.route('/dashboard')
  .get(passportJWT, async (req, res, next) => {
    ...
    // const contact = await req.user.contact;
    // const email = await req.user.email;
    // const location = await req.user.location;
    // const food = req.user.food;
    // const item = req.user.item;
    await req.user;
  ...
  Thank you for your contribution!`;
    ...
    await Executive.findOneAndUpdate({'_id':req.user.id, 'food._id':req.body.id},{$set: {'food.$.status':true}}, async function(err1, exedata){
      ...
      console.log(exedata);
      res.json({executive_data:exedata});
    });
