...
class foodDonate extends React.Component{
    ...
    postEditData = async()=>{
        const res=await axios.post("http://localhost:8080/home/food", {donatedBy:this.state.donatedBy,
    ...
    render(){
        if(this.state.executiveAssigned){
          return <Redirect to = {{ pathname: "/Donate",
        state:{executive:this.state.executive,donation:this.state.donation}
        }} />;
        ...
        return(
            <div className="foodcontainer"><LogHeader/>
            <div className="container formcontainer border border-white"><img src={logo}/> DONATE FOOD
            <form onSubmit={this.onFormSubmit}><div className="form-group">
